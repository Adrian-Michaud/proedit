...
static int ProcessKey(UINT msg, WPARAM wParam, LPARAM lParam, 
                      char *keyState, int *mode);
...
static long macroIndex;
static long macroSize;
static int  macro;
...
#if 1
   proeditWindow.hFont =  CreateFont(16,
                                     12, 0, 0, 
                                     FW_NORMAL, 0, 0, 0,   
...
for (i=0; i<16; i++)
    ...
    if (i & AWS_FG_R) red   = 128;
    if (i & AWS_FG_G) green = 128;
    if (i & AWS_FG_B) blue  = 128;
...
int draw_x, draw_y;
   ...
   memcpy(&original[line*screenXD*SCR_PTR_SIZE],
                 &buffer  [line*screenXD*SCR_PTR_SIZE], screenXD*SCR_PTR_SIZE);
    ...
    for (x=0;x<screenXD;x++)
         {
         line_text[counter] = buffer[(x*SCR_PTR_SIZE)+(line*(screenXD*SCR_PTR_SIZE))];  
...
static void DrawFontString(int x, int y, char *str, int len)
{
   ExtTextOut(proeditWindow.hdc, x,y, ETO_OPAQUE, NULL, str, len, fontSpacing); 
}
...
int ch;
   ...
   for ( ; ; )
      ...
      if (!cnt)
         {
         WaitForSingleObject(proeditWindow.event, INFINITE);
         continue;
         }
...
int ProcessCTRLKey(int ch)
{
   ...
   switch(ch)
      {
      case 'A':return(ED_CTRL_A); 
      case 'B':return(ED_CTRL_B); 
      case 'C':return(ED_CTRL_C); 
      case 'D':return(ED_CTRL_D); 
      case 'E':return(ED_CTRL_E); 
      case 'F':return(ED_CTRL_F); 
      case 'G':return(ED_CTRL_G); 
      case 'H':return(ED_CTRL_H); 
      case 'I':return(ED_CTRL_I); 
      case 'J':return(ED_CTRL_J); 
      case 'K': return(ED_CTRL_K); 
      case 'L': return(ED_CTRL_L); 
      case 'M': return(ED_CTRL_M); 
      case 'N': return(ED_CTRL_N); 
      case 'O': return(ED_CTRL_O); 
      case 'P': return(ED_CTRL_P); 
      case 'Q': return(ED_CTRL_Q); 
      case 'R': return(ED_CTRL_R); 
      case 'S': return(ED_CTRL_S);  
      case 'T': return(ED_CTRL_T); 
      case 'U': return(ED_CTRL_U); 
      case 'V': return(ED_CTRL_V); 
      case 'W': return(ED_CTRL_W); 
      case 'X': return(ED_CTRL_X); 
      case 'Y': return(ED_CTRL_Y); 
      case 'Z': return(ED_CTRL_Z); 
      }
...
/*#                                                                         #*/
/*###########################################################################*/
void OS_SetWindowTitle(char *title)
{
   SetWindowText(proeditWindow.hWndMain, title);
}
